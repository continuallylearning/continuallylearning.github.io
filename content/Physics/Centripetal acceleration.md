...
&= \begin{bmatrix} r \cos(\theta(t)) \\ r \sin(\theta(t)) \end{bmatrix} \quad \text{(Substitute in polar coordinates)} \\
&= \begin{bmatrix} r \cos(\omega t) \\ r \sin(\omega t) \end{bmatrix} \quad \text{(Where } \theta(t) = \omega t, \quad \omega = 2 \pi f \text{)}
\end{aligned}
$$
...
\begin{aligned}
v(t) &:= \frac{dp}{dt}(t) \\
&= \begin{bmatrix} -r \omega \sin(\omega t) \\ r \omega \cos(\omega t) \end{bmatrix} \quad \text{(Calculate time derivative)}\\
&= \omega \begin{bmatrix} -y(t) \\ x(t) \end{bmatrix} \quad \text{(Pull out } \omega \text{)}
\end{aligned}
$$
...
$$
\begin{aligned}
a(t) &= \begin{bmatrix} -r \omega^2 \cos(\omega t) \\ -r \omega^2 \sin(\omega t) \end{bmatrix} \quad \text{(Time derivative of } v(t) \text{)}\\
&= \omega^2 \begin{bmatrix} -x(t) \\ -y(t) \end{bmatrix} \quad \text{(Pull out } \omega^2 \text{)} \\
&= -\omega^2 p(t) \quad \text{(Substitute in } -p(t) \text{)}\\ 
&= -\omega^2 r \hat{p}(t) \quad \text{(where } p(t) := r \hat{p}(t) \text{)}
\end{aligned}
$$
...
We can also define $a(t)$ using linear velocity $v(t)$ instead of using angular velocity $\omega(t)$:

$$
\begin{aligned}
a(t) &= -\omega^2 r \hat{p}(t) \\
&= -\left(\frac{v}{r}\right)^2 r \hat{p}(t) \quad \text{(Substitute } \omega = \frac{v}{r} \text{)} \\
&= -\frac{v^2}{r} \hat{p}(t)
\end{aligned}
$$
